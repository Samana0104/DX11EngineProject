...
void MyMesh2D::SetVertexColor(const int _colorIdx, const vec4 _color)
{
	try
	{
...
void MyMesh2D::SetVertexUV(const int _uvIdx, const vec2 _uv)
{
	try
	{
		mUV.at(_uvIdx) = _uv;
...
void MyMesh2D::MoveVertex(const int _vertexIdx, const vec2 _pos)
{
	try
	{
...
void MyMesh2D::RotateVertex(const int _vertexIdx, const float _angle)
{
	try
	{
