...
void MyTransformer2D::AddTranslationInMat(const vec2 _pos)
{
	SetTranslationInMat(mLocation + _pos);
}

void MyTransformer2D::AddRotationInMat(const float _angle)
{
	SetRotationInMat(mAngle + _angle);
}

void MyTransformer2D::AddScaleInMat(const vec2 _scale)
{
	SetScaleInMat(mScale + _scale);
}

void MyTransformer2D::SetTranslationInMat(const vec2 _pos)
{
	mLocation = _pos;
...
void MyTransformer2D::SetRotationInMat(const float _angle)
{
	if (_angle >= 360.f)
	{
		// [-360, 360]
...
void MyTransformer2D::SetScaleInMat(const vec2 _scale)
{
	mScale = _scale;
	CalculateScaleRotationMat();
}
