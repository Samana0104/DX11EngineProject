...
Mesh::Mesh(MeshShape meshShape)
    : m_meshShape(meshShape)
{}

void Mesh::ReserveVertexSize(size_t vertexCount)
{
    if (vertexCount <= 2)
        vertexCount = 3;
...
void Mesh::AddVertexAndUV(const vec2 vertex, const vec2 uv)
{
    // m_vertices.emplace_back(vertex);
    //  m_uv.emplace_back(uv);
}

void Mesh::AddVertexIndex(std::initializer_list<size_t> index)
{
    for (auto idx : index)
    {
...
void Mesh::UpdateRenderVertices(const mat3& matrix, const vec4& color)
{
    vec3              dim2ToDim3;
...
void Mesh::Draw(const mat3& matrix, const vec4 color)
{
