...
Transform2D& Transform2D::AddLocation(const vec2 _pos)
{
    SetLocation(mLocation + _pos);
    return *this;
}

Transform2D& Transform2D::AddRotation(const float _angle)
{
    SetRotation(mAngle + _angle);
    return *this;
}

Transform2D& Transform2D::AddScale(const vec2 _scale)
{
    SetScale(mScale + _scale);
    return *this;
}

Transform2D& Transform2D::AddScaleAsRatio(const float _ratio)
{
    SetScale(mScale * _ratio);
    return *this;
}

Transform2D& Transform2D::SetLocation(const vec2 _pos)
{
    mLocation     = _pos;
    mTRSMat[2][0] = _pos.x;
    mTRSMat[2][1] = _pos.y;
...
Transform2D& Transform2D::SetRotation(const float _angle)
{
...
Transform2D& Transform2D::SetScale(const vec2 _scale)
{
...
void Transform2D::MultiplyToTRSMat(const mat3& _mat)
{
...
{
    return {mLocation.x - mScale.x * 0.5f,
...
{
    return {-mScale.x * 0.5f, mScale.y * 0.5f, mScale.x * 0.5f, -mScale.y * 0.5f};
}
...
vec2 Transform2D::CalculateTRSAsVec(const vec2 _pos)
{
    const vec2 scaledVec  = Transform2D::ResizeScale(_pos, mScale);
    const vec2 rotatedVec = Transform2D::RotateAsAngle(scaledVec, mAngle);
    return rotatedVec + mLocation;
}

RECT_F Transform2D::CartesianToPixelRect(const RECT_F _rect)
{
...
vec2 Transform2D::CartesianToNDC(const vec2 _pos)
{
    return (_pos / mCartesianSize) * 2.f;
}

vec2 Transform2D::CartesianToPixel(const vec2 _pos)
{
...
vec2 Transform2D::PixelToCartesian(const vec2 _pos)
{
...
vec2 Transform2D::PixelToNDC(const vec2 _pos, const vec2 _rectSize)
{
    const vec2 normalizedVec = _pos / _rectSize;
    return {normalizedVec.x * 2.f - 1.f, -(normalizedVec.y * 2.f - 1.f)};
}

vec2 Transform2D::CartesianToPolar(const vec2 _pos)
{
    return {glm::length(_pos), glm::atan(_pos.y / _pos.x)};
}

vec2 Transform2D::PolarToCartesian(const vec2 _pos)
{
    // _pos = (r,theta)
    return {_pos.x * glm::cos(_pos.y), _pos.x * glm::sin(_pos.y)};
}

vec2 Transform2D::RotateAsAngle(const vec2 _pos, const float _angle)
{
    return RotateAsRadian(_pos, glm::radians(_angle));
}

vec2 Transform2D::RotateAsRadian(const vec2 _pos, const float _radian)
{
    return {_pos.x * glm::cos(_radian) - _pos.y * glm::sin(_radian),
            _pos.x * glm::sin(_radian) + _pos.y * glm::cos(_radian)};
}

vec2 Transform2D::ResizeScale(const vec2 _pos, const vec2 _scale)
{
    return {_pos.x * _scale.x, _pos.y * _scale.y};
}

vec2 Transform2D::Slerp(const vec2 _pos1, const vec2 _pos2, float t)
{
    /*
